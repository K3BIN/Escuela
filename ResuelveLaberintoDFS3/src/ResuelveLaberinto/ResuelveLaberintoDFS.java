...
public class ResuelveLaberintoDFS extends JFrame {
...
// ***********************
    ...
    boolean planea() {
        ...
        // UBICA EN DONDE SE ENCUENTRA LA POSICIÓN META DEL MÓVIL
        for (ren = 0; ren < renMax; ren++) {
            for (col = 0; col < colMax; col++) {
                if (matriz[ren][col] == 3) {
                    metaRen = ren;
        ...
        if (inicioRen == -1 || inicioCol == -1 || metaRen == -1 || metaCol == -1) {
        ...
        // INICIA LA BÚSQUEDA EN AMPLITUD
        pila.inserta(inicial);
        while (continua && pila.vacia() == false) {
            Estado actual = (Estado) pila.retira();
    ...
    void guardaPlan(int meta, ArrayList visitados, int metaRen, int metaCol) {
        ...
        for (int j = longitudPlan - 2; j > -1; j--) {
            ...
            if( plan.ren == metaRen && plan.col == metaCol )
                continue;
    ...
    public static void main(String args[]) {
        ...
        ResuelveLaberintoDFS ventanaPrincipal = new ResuelveLaberintoDFS();
