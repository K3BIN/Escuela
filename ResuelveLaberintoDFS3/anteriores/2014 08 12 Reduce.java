...
public class Reduce extends JFrame {

JButton bRegla = new JButton("Rules");
    ...
    JTextArea tBitacora = new JTextArea("");

    JLabel eForma = new JLabel("Shape:");
    JLabel eResultado = new JLabel("Result:");
    JLabel eBitacora = new JLabel("Log:");
...
// POSICION DE LA VENTANA ACTUAL
    ...
    JTable tablaRegla;
    JScrollPane scrollRegla ;

    JScrollPane panelBitacora = new JScrollPane();
...
// ACCIONES
    ...
    ActionListener aRegla = new ActionListener() {
        public void actionPerformed(ActionEvent e) {
            String nomArch = eligeArchivo("Select the rules to be applied");
...
DefaultTableModel modelo = (DefaultTableModel)tablaRegla.getModel();

        try {
            BufferedReader lexico = new BufferedReader(new FileReader(nomArch));

//            BufferedReader lexico = new BufferedReader(new FileReader(".\\datos\\reglasI.txt"));
            int numeroRegla = 0;
            ...
            while (lexico.ready()) {
                ...
                if( lineaLexico.charAt(0) == '#' )
                    regla.nombre = lineaLexico;
                ...
                unaRegla.append("" +
                        (numeroRegla < 10 ? ("00" + numeroRegla)
                        : numeroRegla < 99 ? ("0" + numeroRegla)
                        : numeroRegla) + " " + regla.nombre
                        +"     ");
                        // + "    (" + regla.avance + ") ");
...
Object datos[] = new Object[4];
        datos[0] = ""+numeroRegla;
        datos[1] = ""+regla.nombre;
        datos[2] = ""+regla.antes+" --> "+regla.despues;
        datos[3] = true;
...
tBitacora.append("    Loaded " + uReglas + " rules\n");
...
//                System.out.println("ANTES: " + cadena);
                if (cadena.toString().substring(nuevo.posicion).startsWith(antes)) {
                    ...
                    tBitacora.append("Rule: " + iRegla+" "+reglas.get(iRegla).nombre+"  "
                        + reglas.get(iRegla).antes+" -> "+reglas.get(iRegla).despues+"\n");

                    nuevo.nodo = ++nodo;
                    nuevo.padre = actual.nodo;
                    System.out.println("nodo: " + nuevo.nodo + "   padre: " + nuevo.padre);

                    tBitacora.append("node: " + nuevo.nodo + "   father: " + nuevo.padre+"\n");
                    ...
                    tBitacora.append("in: " + cadena+"\n");

                    System.out.println("DESPUES: " + nueva);

                    tBitacora.append("out: " + nueva+"\n\n");
...
//System.out.println("ALTO "+YPmax+" ANCHO "+XPmax);
        ...
        bRegla.setFont(fNormal);
        cont.add(bRegla);
        bRegla.setBounds(700, 50, 80, 30);


        eForma.setBounds(20, 77 ,100 ,20);
        cont.add(eForma);

        tNumeroForma.setFont(fNormal);
        cont.add(tNumeroForma);
        tNumeroForma.setLineWrap(true);
        tNumeroForma.setBorder(BorderFactory.createMatteBorder(
                                    1, 1, 3, 3, Color.lightGray ));
        tNumeroForma.setBounds(20, 95, 380, 60);

        tablaRegla = new JTable();
        tablaRegla.setModel(new DefaultTableModel(
        new Object [][] {
              // {null, null, null, null}
            },
        new String [] {
        ...
        tablaRegla.getColumnModel().getColumn(0).setMinWidth(0);
        tablaRegla.getColumnModel().getColumn(0).setPreferredWidth(30);
        tablaRegla.getColumnModel().getColumn(0).setMaxWidth(30);

        tablaRegla.getColumnModel().getColumn(1).setMinWidth(0);
        tablaRegla.getColumnModel().getColumn(1).setPreferredWidth(140);
        tablaRegla.getColumnModel().getColumn(1).setMaxWidth(140);

        tablaRegla.getColumnModel().getColumn(2).setMinWidth(0);
        tablaRegla.getColumnModel().getColumn(2).setPreferredWidth(170);
        tablaRegla.getColumnModel().getColumn(2).setMaxWidth(170);

        tablaRegla.getColumnModel().getColumn(3).setMinWidth(0);
        tablaRegla.getColumnModel().getColumn(3).setPreferredWidth(20);
        tablaRegla.getColumnModel().getColumn(3).setMaxWidth(20);

        scrollRegla = new JScrollPane(tablaRegla);
        cont.add(scrollRegla);
        scrollRegla.setBounds(20, 170,380,120);


        bTransforma.setFont(fNormal);
        cont.add(bTransforma);
        bTransforma.setBounds(145, 305, 100, 30);

        eBitacora.setBounds(20, 342, 100, 20);
        cont.add(eBitacora);

//        tBitacora.setText("bitacora");
        tBitacora.setFont(fNormal);
        cont.add(tBitacora);
        tBitacora.setLineWrap(true);
        tBitacora.setBorder(BorderFactory.createMatteBorder(
                                    1, 1, 3, 3, Color.lightGray ));

        panelBitacora = new JScrollPane(tBitacora);

        panelBitacora.setBounds(20, 360, 380, 150);
        cont.add(panelBitacora);

//        tResultado.setText("RESULTADO");
        eResultado.setBounds(430, 77, 100, 20);
        cont.add(eResultado);

        tResultado.setFont(fNormal);
        cont.add(tResultado);
        tResultado.setLineWrap(true);
        tResultado.setBorder(BorderFactory.createMatteBorder(
                                    1, 1, 3, 3, Color.lightGray ));
//        tResultado.setBounds(430, 140, 350, 60);
        tResultado.setBounds(430, 95, 350, 60);


        eResolucion.setFont(fNormal);
        cont.add(eResolucion);
//        eResolucion.setBounds(20, 95, 150, 30);
        eResolucion.setBounds(450, 165, 150, 30);

        tResolucion.setFont(fNormal);
        cont.add(tResolucion);
        tResolucion.setEditable(false);
//        tResolucion.setBounds(90, 95, 30, 30);
        tResolucion.setBounds(520, 165, 30, 30);

        bAumenta.setFont(fNormal);
        cont.add(bAumenta);
//        bAumenta.setBounds(125, 95, 41, 13);
        bAumenta.setBounds(555, 165, 41, 13);
        bAumenta.addActionListener(aAumenta);

        bReduce.setFont(fNormal);
        cont.add(bReduce);
//        bReduce.setBounds(125, 112, 41, 13);
        bReduce.setBounds(555, 182, 41, 13);
...
//        bVisualiza.setBounds(500, 95, 150, 30);
        bVisualiza.setBounds(500, 205, 150, 30);

        cont.add(digital);
        digital.setBounds(430, 250, 260, 260);


        bSalir.setFont(fNormal);
        cont.add(bSalir);
        bSalir.setBounds(700, 500, 80, 30);

        // ASIGNA LAS ACCIONES
        bCarga.addActionListener(aCarga);
        bRegla.addActionListener(aRegla);
